...
Eigen::Matrix4d ICP(PointCloudT::Ptr sourceCloud, const PointCloudT::Ptr targetCloud, int iterations, Pose startingPose) {
  Eigen::Matrix4d transformation_matrix = Eigen::Matrix4d::Identity();
  PointCloudT::Ptr icp_cloud (new PointCloudT);
  PointCloudT::Ptr transformedSourceCloud (new PointCloudT);
  ...
  pcl::transformPointCloud(*sourceCloud, *transformedSourceCloud, initTransform);
  
  pcl::IterativeClosestPoint<PointT, PointT> icp;
  icp.setMaximumIterations(iterations);
  icp.setInputSource(transformedSourceCloud);
  icp.setInputTarget(targetCloud);
  icp.align(*icp_cloud);
  
  if (icp.hasConverged()) {
    cout << "\nICP has converged, score is " << icp.getFitnessScore() << endl;
	transformation_matrix = icp.getFinalTransformation().cast<double>() * initTransform;
  } else {
    cout << "\n ICP did not converge" << endl;
  }
...
int main(){
    ...
    PointCloudT::Ptr prevScanCloud(new PointCloudT);
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			Eigen::Matrix4d poseTransformMatrix = ICP(cloudFiltered, mapCloud, 10, pose);
            pose = getPose(poseTransformMatrix);
          	//

			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
          	PointCloudT::Ptr transformedScanCloud (new PointCloudT);
          	pcl::transformPointCloud(*cloudFiltered, *transformedScanCloud, poseTransformMatrix);

			viewer->removePointCloud("scan");
			// TODO: Change `scanCloud` below to your transformed scan
			renderPointCloud(viewer, transformedScanCloud, "scan", Color(1,0,0) );
